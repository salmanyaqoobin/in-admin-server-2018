...
describe('## Misc', function () {
  ...
  describe('# Error Handling', function () {
    ...
    it('should handle express validation error - name is required', function (done) {
      request(app).post('/api/users').send({
        mobileNumber: '1234567890'
      }).expect(httpStatus.BAD_REQUEST).then(function (res) {
        expect(res.body.message).to.equal('"name" is required and "email" is required and "password" is required');
        done();
      }).catch(done);
    });
  });
